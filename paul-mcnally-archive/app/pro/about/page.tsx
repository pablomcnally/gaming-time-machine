...
export default function ProfessionalAbout() {
  return <div className="pro-container pro-page">
    ...
    <div className="pro-about-layout"><aside><p className="pro-eyebrow">On this page</p><nav aria-label="About sections">{aboutContent.panels.map((panel, index) => <a href={`#about-${index}`} key={panel.title}>{panel.title.toLowerCase()}</a>)}<a href="#podcast">Podcast interview</a><a href="#experience">Career timeline</a></nav><Link className="pro-text-link" href="/pro/contact">Get in touch</Link></aside>
      ...
      <section className="pro-about-section" id="podcast"><h2>Podcast interview</h2><p>Maximum Power Up&apos;s two-hour conversation traces Paul&apos;s path through games journalism, from ST Action and Mega Action to PlayStation Pro, GamePro UK and the stories behind the magazines.</p><Link className="pro-text-link" href="/pro/features/maximum-power-up-paul-mcnally-interview">Listen to Episode 103</Link></section>
