...
import { getProfessionalSummaries, professionalKinds, type ProfessionalKind } from "../../lib/professional";
...
export const collectionDetails = {
  ...
  opinion: { title: "Opinion Pieces", description: "Columns, commentary and opinion on games, technology and the world around them." },
...
export function CollectionPage({ kind, category }: { kind?: ProfessionalKind; category?: ReviewCategory }) {
  ...
  return <div className="pro-container pro-page">
    ...
    <nav className="pro-collection-tabs" aria-label="Work collections">
      {[{ label: "All work", value: undefined, href: "/pro/work" }, ...professionalKinds.map((value) => ({ label: collectionDetails[value].title, value, href: `/pro/${value}` }))].map((item) => <Link key={item.href} href={item.href} aria-current={item.value === kind ? "page" : undefined}>
    ...
    {articles.length ? <WorkLibrary key={`${kind || "all"}/${category || "all"}`} articles={articles} /> : <section className="pro-empty"><h2>No pieces published yet.</h2></section>}
  </div>;
}
